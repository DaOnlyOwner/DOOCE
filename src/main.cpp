...
PYBIND11_MODULE(dooce, m)
{
	...
	m.def("make_gameplay_st", [](const game& g, float minutes_to_play, int cap_tt)
		{
			return std::unique_ptr<gameplay>(static_cast<gameplay*>(new gameplay_st(minutes_to_play, g, cap_tt)));
		});
	...
	py::class_<timer>(m, "Timer")
		...
		.def("time_is_up", &timer::time_is_up)
		.def("action_now", &timer::action_now)
		.def("action_took_time_of_span", &timer::action_took_time_of_span)
		.def("ms_left", &timer::ms_left)
		.def("sec_left", &timer::sec_left)
		.def("min_left", &timer::min_left);

	py::class_<move_info>(m, "MoveInfo")
		.def(py::init<>())
		.def_readonly("depth", &move_info::depth)
		.def_readonly("pv", &move_info::principal_variation)
		.def_readonly("score", &move_info::score);
		...
		.def("is_in_check", [](game& g)
			{
				...
				return g.get_game_context().turn == color::white ? g.from_dooce_algebraic_notation<color::white>(mv) : g.from_dooce_algebraic_notation<color::black>(mv); })
				.def("from_move_to_dooce_algebraic_notation", &game::from_move_to_dooce_algebraic_notation);
}